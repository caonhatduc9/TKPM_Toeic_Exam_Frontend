...
const MyTable = ({ ...props }) => {
    ...
    const row = (item, index) => {
        if (type == "managetest")
            ...
            return (<><td>{item.id}</td><td>{item.name}</td><td>{item.email}</td><td>{item.phone}</td><td>{item.dob}</td></>)
        else if (type == "manageblog")
            return (<><td>{index + 1}</td><td className={style.blog} onClick={() => { navigate("/blog/" + item.id) }}>{item.title}</td><td>{item.date}</td><td>{item.watch}</td><td>
                <div className={style.edit}>
                    <span>Edit</span>
                </div>
                <div className={style.delete}>
